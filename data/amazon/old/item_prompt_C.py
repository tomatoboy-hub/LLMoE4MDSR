...
data = json.load(open("./handled/item2attributes_C.json", "r"))
...
id_map = json.load(open("./handled/id_map.json", "r"))["item_dict"]["2"]["str2id"]
title_data = {}
...
with open("./handled/title_C.pkl", "wb") as f:
...
json.dump(item_data, open("./handled/item_str_C_truncate.json", "w"))
...
save_data("item_str_C_truncate.jsonline", json_data)
...
if os.path.exists("./handled/item_emb_C.pkl"):
    item_emb = pickle.load(open("./handled/item_emb_C.pkl", "rb"))
else:
    item_emb = {}
    ...
    try:
        ...
        with tqdm(total=len(keys_to_process), desc="Generating Embeddings") as pbar:
            for i, key in enumerate(keys_to_process):
                ...
                if (i + 1) % 100 == 0:
                    pickle.dump(item_emb, open("./handled/item_emb_C.pkl", "wb"))
...
id_map = json.load(open("./handled/id_map.json", "r"))["item_dict"]["2"]["id2str"]
emb_list = []
for id in range(1, len(id_map)+1):
...
pickle.dump(emb_list, open("./handled/itm_emb_np_C.pkl", "wb"))
