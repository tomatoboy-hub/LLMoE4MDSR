...
class AmazonHandler():
    def __init__(self,domains:list):
        ...
        self.data = []
        self.domain_set = {domain_num : {"user": [], "item": []} for domain_num in range(len(domains)) }
    ...
    def load_data(self):
        all_data = []
        for i, domain_name in self.domains:
            try:
...
if __name__ == "__main__":
    ...
    handler = AmazonHandler(domains=[domain_A, domain_B, domain_C])
    handler.load_data()
    handler.filter_time(t_min=1514736000, t_max=1577808000)
    handler.filter_Kcore(user_core=5, item_core=3)
    handler.id_map()
    ...
    with open(f"{output_dir}id_map.json", "w") as f:
        all_maps = {"user_dict":handler.user_dict, "item_dict":handler.item_dict}
        json.dump(all_maps, f, indent=4)
        print(f"Saved id_map.json")
    
    with open(f"{output_dir}amazon_all.pkl", "wb") as f:
        pickle.dump((handler.final_data, handler.final_domain), f)
        print(f"Saved amazon_all.pkl")

    
    handler.export_sequences_for_domains(
        domain_ids=[0, 1, 2], 
        output_path=f"{output_dir}cloth_sport_fashion.pkl"
    )
